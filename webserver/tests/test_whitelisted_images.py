...
class TestWhitelistedImageModelValidation:
    def test_container_validate_missing_registry(self, client):
        ...
        with pytest.raises(ContainerRegistryException) as exc:
            WhitelistedImage.validate({"name": "test", "registry": "non-existent.io", "tag": "latest"})
    ...
    def test_whitelist_image_only_matches_any_version(self, registry):
        ...
        img = WhitelistedImage(name="img1", registry=registry, tag=None, sha=None)
        img.add()
        assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img1:any-tag") is True
        assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img1@sha256:{"a"*64}") is True
    ...
    def test_whitelist_image_and_tag_matches_specific_tag(self, registry):
        ...
        img = WhitelistedImage(name="img2", registry=registry, tag="v1", sha=None)
        img.add()
        assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img2:v1") is True
        assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img2:v2") is False
    ...
    def test_whitelist_image_tag_and_sha_matches_direct_sha_request(self, registry):
        ...
        s1 = "sha256:" + "1" * 64
        img = WhitelistedImage(name="img3", registry=registry, tag="v1", sha=s1)
        img.add()
        ...
        with mock.patch("app.models.registry.Registry.get_registry_class") as mock_reg_class:
            ...
            # Full match: Tag + SHA
            assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img3:v1@{s1}") is True
            # Tag matches, SHA doesn't
            assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img3:v1@sha256:different") is False
            # SHA matches, Tag doesn't (whitelisting is tag-restricted)
            assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img3:v2@{s1}") is False
            # Only SHA provided (whitelisting is tag-restricted)
            assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img3@{s1}") is False
    ...
    def test_whitelist_image_tag_and_sha_matches_remote_resolution(self, registry):
        ...
        with mock.patch("app.models.registry.Registry.get_registry_class") as mock_reg_class:
            ...
            assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img3:v1") is True
            # Tag matches, remote SHA doesn't match
            mock_client.get_tag_sha.return_value = "sha256:different"
            assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img3:v1") is False
    ...
    def test_whitelist_image_and_sha_matches_direct_sha_request(self, registry):
        ...
        s4 = "sha256:" + "4" * 64
        img = WhitelistedImage(name="img4", registry=registry, tag=None, sha=s4)
        img.add()
        # Match by SHA directly
        assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img4@{s4}") is True
        # Match by Tag + SHA
        assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img4:any-tag@{s4}") is True
    ...
    def test_whitelist_image_and_sha_matches_remote_resolution(self, registry):
        ...
        with mock.patch("app.models.registry.Registry.get_registry_class") as mock_reg_class:
            ...
            assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img4:any-tag") is True
            # Remote SHA doesn't match
            mock_client.get_tag_sha.return_value = "sha256:different"
            assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img4:any-tag") is False
    ...
    def test_whitelist_ignores_remote_existence_check(self, registry):
        ...
        img = WhitelistedImage(name="img-exists", registry=registry, tag="v1", sha=None)
        img.add()
        ...
        with mock.patch("app.models.registry.Registry.get_registry_class") as mock_reg_class:
            ...
            assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img-exists:v1") is True
            
            # Whitelisted by SHA, but doesn't exist remotely
            s1 = "sha256:" + "s" * 64
            img_sha = WhitelistedImage(name="img-sha", registry=registry, tag=None, sha=s1)
            img_sha.add()
            mock_client.has_image_tag_or_sha.return_value = False
            assert WhitelistedImage.validate_image_whitelisted(f"{registry.url}/img-sha@{s1}") is True
