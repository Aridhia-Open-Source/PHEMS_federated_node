...
def bootstrap_platform_accounts(admin_token:str):
    ...
    # overwrite it. This is what makes `admin` genuinely operator-owned -- nothing programmatic
    # authenticates as it any more, so changing its password cannot break the platform.
...
while True:
    ...
    for kc_pod in watcher.stream(
        ...
        try:
            ...
            #
            # `<` rather than `!=`: with exact equality, scaling Keycloak up without also updating
            # KC_REPLICAS meant this count could never match again and the controller wedged
            # permanently, silently skipping every realm migration.
