...
from app.github import GithubClient, GithubAPI
...
class GithubSensorConfig:
    ...
    @cached_property
    def enabled(self):
        return os.environ.get("GH_SENSORS_ENABLED", "").lower() == "true"
    ...
    def __init__(self):
        ...
        self.backend_api_uri = os.environ.get("BACKEND_API_URI", "").rstrip("/")
        self.kc_user = os.environ.get("DAGSTER_KC_USER", "")
        self.kc_password = os.environ.get("DAGSTER_KC_PASSWORD", "")
...
@dg.run_status_sensor(
    ...
    default_status=dg.DefaultSensorStatus.STOPPED,   # dg.DefaultSensorStatus.RUNNING
...
def github_pr_comment_op(context: OpExecCtx):
    ...
    gh.add_pull_request_comment("placeholder/placeholder", pr_number, body)
...
@dg.sensor(
    job=k8s_pipes_job,
    minimum_interval_seconds=MIN_SENSOR_INTERVAL_SECONDS,
    default_status=dg.DefaultSensorStatus.RUNNING,  # TODO: if gh_config.enabled else dg.DefaultSensorStatus.STOPPED,
)
def github_pull_request_polling_sensor(context):
    # backend_token = _backend_login().json()["token"]
    response = requests.get(
        f"{gh_config.backend_api_uri}/repositories",
    ...
    gh = GithubAPI(GithubClient(token=gh_config.token), logger=context.log)

    for repo in repos:
        base_branch, merge_cursor = repo["base_branch"], repo["merge_cursor"]
        new_prs = gh.get_new_merged_pulls(repo['path'], base_branch, merge_cursor)
        watched_prs = gh.filter_prs_by_watch_dir(new_prs, repo["watch_dir"])
        context.log.info(f"Fetched new PRs for repository: {repo['uri']}")
        ...
        for pr in watched_prs:
            context.log.info(f"Processing PR #{pr['number']} for repository: {repo['uri']}")
            ...
            content = gh.get_file_contents(repo['path'], filename, ref=pr["merge_commit_sha"])
            spec = json.loads(content)["spec"]
            spec['image'] = spec.get('image') or spec.get('docker_image')  # FIXME: perform validation and skip on invalid schema
        ...
        merge_cursor = max(pr["pull_request"]['merged_at'] for pr in new_prs)

        requests.patch(
            f"{gh_config.backend_api_uri}/repositories/{repo['id']}",
            json={"merge_cursor": merge_cursor, "polled_at": utc_now()},
            timeout=10,
        )
...
def _backend_login():
    response = requests.post(
        f"{gh_config.backend_api_uri}/login",
        data={"username": gh_config.kc_user, "password": gh_config.kc_password},
    )
    response.raise_for_status()
    return response
