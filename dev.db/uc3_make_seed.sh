...
# UC3 Seed Script (master orchestrator)
# Drops, creates schema, seeds synthetic haemophilia data, and registers in backend API
...
echo -e "${BLUE}=== UC3 Haemophilia OMOP Database Seed ===${NC}"
echo ""
...
echo -e "${BLUE}[1/3] Dropping existing UC3 schemas...${NC}"
"$SCRIPT_DIR/uc3_drop.sh"
...
PGPASSWORD="$DATASET_PASSWORD" "$PYTHON" "$SCRIPT_DIR/uc3_seed.py" "$SCRIPT_DIR/omop54_fields.csv" | \
...
echo -e "${GREEN}✓ UC3 OMOP database fully seeded!${NC}"
echo ""
echo -e "${BLUE}To verify, run: $PYTHON uc3_read.py${NC}"
echo ""
