    ...
    function required_env(name::String)::String
        ...
        value === nothing && error("$name unset")
        return value
    end


    function collect_datasets(dataset_dir::String)
        csv_files = filter(f -> endswith(f, ".csv"), readdir(dataset_dir; join=true))
        isempty(csv_files) && error("No .csv files found in $dataset_dir")
